...
fn time_algo(algo: Algo, frames: &[GrayImage], cascade: Option<&Cascade>) -> (f64, usize) {
    ...
    // CNN 模板权重慢;用 RSFACE_BENCH_RUNS 控,默认 3。
    let n_runs: usize = std::env::var("RSFACE_BENCH_RUNS")
        .ok()
    ...
    for run in 0..n_runs {
        ...
        // 当 n_runs = 1(快速冒烟),第 0 轮也是有效样本;
        // 否则丢弃第 0 轮(warmup)只保留后续轮。
        if n_runs > 1 && run == 0 {
            // skip warmup
        } else {
            samples_ms.push(per * 1e3);
        }
        total_dets = run_dets;
    }
    samples_ms.sort_by(|a, b| a.partial_cmp(b).unwrap());
    let median = if samples_ms.is_empty() {
        0.0
    } else {
...
fn main() {
    ...
    let lena_img = ["lena.jpg", "lena.pgm", "pgm/lena.pgm"]
        .iter()
        .filter_map(|n| find_test(n))
        .find_map(|p| load_gray(&p));
    ...
    println!(
        "  bbb-360-10s.mp4 : {} frames (target=1)",
        video_frames.len()
    );
    match &cascade {
        Some(c) => println!(
            "  cascade         : {} features, {} stages, window {}x{}",
            c.features.len(),
            c.stages.len(),
            c.window_w,
            c.window_h
    ...
    for algo in Algo::all() {
        if let Some(ref f) = filter {
            if algo.name() != f.as_str() {
                continue;
            }
        }
        // Haar 必须有 cascade;CNN 不需要。
        // CNN 模板权重在 512x512 单帧 ~10s,所以给 CNN 一张小图(64x64)作为
        // 性能基线参考,真实大图数字标 "slow" 标签。
        let lena_ms = if cascade.is_some() && lena_img.is_some() && matches!(algo, Algo::Haar) {
            lena_img.as_ref().map(|img| time_algo(*algo, &[img.clone()], cascade.as_ref()).0)
        } else if matches!(algo, Algo::Cnn) {
            let small = GrayImage::new(64, 64);
            Some(time_algo(*algo, &[small], cascade.as_ref()).0)
        } else {
            None
        };
        let tp_ms = if cascade.is_some() && tp_img.is_some() && matches!(algo, Algo::Haar) {
            tp_img.as_ref().map(|img| time_algo(*algo, &[img.clone()], cascade.as_ref()).0)
        } else {
            None
        };
        let (v_ms, v_fps) = if !video_frames.is_empty() && matches!(algo, Algo::Haar) {
            let (ms, _dets) = time_algo(*algo, &video_frames, cascade.as_ref());
            (Some(ms), Some(1000.0 / ms.max(0.001)))
        } else {
            (None, None)
        };
        let lena_label = if matches!(algo, Algo::Cnn) {
            match lena_ms { Some(ms) => format!("{:.2}*", ms), None => "slow".to_string() }
        } else { fmt_ms(lena_ms) };
        let tp_label = if matches!(algo, Algo::Cnn) { "slow".to_string() } else { fmt_ms(tp_ms) };
        let v_label = if matches!(algo, Algo::Cnn) { "slow".to_string() } else { fmt_ms(v_ms) };
        let vfps_label = if matches!(algo, Algo::Cnn) {
            "N/A".to_string()
        } else {
    ...
    if std::env::var("RSFACE_BENCH_WRITE_MD").ok().as_deref() == Some("1") {
        ...
        s.push_str("Each cell: median of 3 runs (per-frame ms; video = ms/frame over 1 frame).\n\n");
        ...
        s.push_str("`cnn/lena` reports a 64x64 baseline (`*` suffix) so the bench finishes in seconds.\n");
        ...
        for (name, l, t, vf, vfps) in &rows {
            let l_str = if name == "cnn" {
                match l { Some(ms) => format!("{:.2}*", ms), None => "slow".into() }
            } else { fmt_ms(l.clone()) };
            let t_str = if name == "cnn" { "slow".to_string() } else { fmt_ms(t.clone()) };
            let vf_str = if name == "cnn" { "slow".to_string() } else { fmt_ms(vf.clone()) };
            let vfps_str = if name == "cnn" {
                "N/A".to_string()
            } else {
            ...
            s.push_str(&format!(
                "| {} | {} | {} | {} | {} |\n",
                name, l_str, t_str, vf_str, vfps_str,
            ));
        }
