...
fn main() {
    ...
    for algo in Algo::all() {
        ...
        let lena_ms = if cascade.is_some() && lena_img.is_some() && matches!(algo, Algo::Haar) {
            lena_img
                .as_ref()
                .map(|img| time_algo(*algo, &[img.clone()], cascade.as_ref()).0)
        ...
        let tp_label = if matches!(algo, Algo::Cnn) {
            "slow".to_string()
        } else {
            fmt_ms(tp_ms)
        };
        let v_label = if matches!(algo, Algo::Cnn) {
            "slow".to_string()
        } else {
            fmt_ms(v_ms)
        };
    ...
    if std::env::var("RSFACE_BENCH_WRITE_MD").ok().as_deref() == Some("1") {
        ...
        for (name, l, t, vf, vfps) in &rows {
            ...
            let t_str = if name == "cnn" {
                "slow".to_string()
            } else {
                fmt_ms(t.clone())
            };
            let vf_str = if name == "cnn" {
                "slow".to_string()
            } else {
                fmt_ms(vf.clone())
            };
