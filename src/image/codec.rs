...
pub fn read_pgm(r: &mut dyn Read) -> std::io::Result<GrayImage> {
    ...
    let mut pos = 2;
    let (w, p) = parse_uint(&buf, pos)
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidData, "bad width"))?;
    pos = p;
    let (h, p) = parse_uint(&buf, pos)
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidData, "bad height"))?;
    pos = p;
    let (maxv, p) = parse_uint(&buf, pos)
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidData, "bad maxval"))?;
    pos = p;
    if maxv != 255 {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
    ...
    if pos >= buf.len() || !buf[pos].is_ascii_whitespace() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidData,
            "expected whitespace before pixel data",
        ));
    }
    pos += 1;
    let needed = w * h;
