...
/// Jacobi convergence: stop a sweep when the largest off-diagonal element is below
/// `|λ_max| * EIGEN_TOL_REL`.
const EIGEN_TOL_REL: f32 = 1e-10;
...
/// returned; eigenvector `k` is column `k` of the returned flat matrix. Input is
/// symmetrised defensively in case of rounding drift.
fn jacobi_symmetric(a: &mut [f32], n: usize) -> (Vec<f32>, Vec<f32>) {
    for i in 0..n {
        for j in 0..i {
