...
impl HaarFeature {
    ...
    pub(crate) fn eval_inbounds(
        ...
        for r in &self.rects {
            ...
            //
            // Tilted features have to clear the rotated-table rims: p1 is
            // `h` columns left of (rx, ry) and p3 is `rw + rh` rows below
            // it. Where they don't, the checked query clips via its zero
            // border — bit-identical on interior rects.
            let sum_f64: f64 = if is_tilted {
                ...
                let s: u64 = if ii.is_wide() {
                    ...
                    // `!is_wide()`.
                    unsafe {
                        ii.rect_sum_unchecked_narrow(rx, ry, rx + rw, ry + rh)
                    }
                };
                s as f64
            };
