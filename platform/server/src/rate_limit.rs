...
mod tests {
    ...
    async fn first_request_allowed_with_full_bucket() {
        ...
        let d = rl.check("1.1.1.1", policy_post().0, policy_post().1).await;
        assert!(d.allowed);
    ...
    async fn capacity_burst_then_refill() {
        ...
        let d1 = rl.check("2.2.2.2", cap, rate).await;
        assert!(d1.allowed);
    ...
    async fn separate_ips_have_independent_buckets() {
        ...
        let _ = rl.check("a", cap, rate).await; // drain a
        let _ = rl.check("a", cap, rate).await; // a denied
        let d = rl.check("b", cap, rate).await;
    ...
    async fn refill_over_time_restores_tokens() {
        ...
        let _ = rl.check("x", cap, rate).await;
        let _ = rl.check("x", cap, rate).await;
        let _ = rl.check("x", cap, rate).await; // 3rd rejected
        tokio::time::sleep(Duration::from_millis(20)).await; // 1000/s * 20ms = 20 tokens restored
        let d = rl.check("x", cap, rate).await;
    ...
    async fn retry_after_is_roughly_deficit_over_rate() {
        ...
        let _ = rl.check("y", cap, rate).await;
        let _ = rl.check("y", cap, rate).await; // denied, expects ~1s
        let d = rl.check("y", cap, rate).await;
        assert!(!d.allowed);
    ...
    async fn bucket_count_tracks_unique_ips() {
        let rl = RateLimiter::new();
        let _ = rl.check("ip1", 10, 1.0).await;
        let _ = rl.check("ip2", 10, 1.0).await;
        let _ = rl.check("ip1", 10, 1.0).await; // reuse
