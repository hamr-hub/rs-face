...
impl S3Client {
    ...
    pub fn put_object(&self, key: &str, content_type: &str, body: Vec<u8>) -> Result<(), S3Error> {
        let body = Some(body);
        self.request("PUT", &format!("/{key}"), &[("content-type", content_type)], body)
            .map(|_| ())
    }
    ...
    pub fn get_object(&self, key: &str) -> Result<(Vec<u8>, String), S3Error> {
        ...
        let mut buf = Vec::new();
        std::io::Read::read_to_end(&mut resp.into_reader(), &mut buf).map_err(|e| S3Error(e.to_string()))?;
        Ok((buf, ct))
    }
    ...
    fn request(
        ...
        // 供签名用的 path
        let sign_path = format!("/{}/{}", url_encode_segment(&self.bucket),
            path_and_query.trim_start_matches('/'));
        ...
        if let Some(ct) = extra_headers.iter().find(|(k, _)| k.eq_ignore_ascii_case("content-type")) {
        ...
        let signed_headers: String = headers.iter()
            .map(|(k, _)| k.as_str()).collect::<Vec<_>>().join(";");
        ...
        let signing_key = hmac_sha256(
            &hmac_sha256(
                &hmac_sha256(
                    &hmac_sha256(format!("AWS4{}", self.secret_key).as_bytes(), &amz_date[..8].as_bytes())?,
        ...
        match result {
            ...
            Err(ureq::Error::Status(_code, resp)) => {
                ...
                if msg.is_empty() {
                    // HEAD 类无 body,用状态码描述
                    Err(S3Error(format!("status {} for {method} {}", _code, sign_path)))
                } else {
                    // 幂等场景:桶已存在等
                    if _code == 409 && method == "PUT" && sign_path.ends_with(&format!("/{}", self.bucket)) {
                        Ok(ureq::Response::new(200, "OK", "").unwrap())
                    } else {
                        Err(S3Error(format!("status {}: {}", _code, truncate(&msg, 300))))
                    }
                }
            }
...
fn hmac_sha256_fixed(key: &[u8], data: &[u8]) -> Result<Vec<u8>, S3Error> {
    let mut mac = Hmac::<sha2::Sha256>::new_from_slice(key)
        .map_err(|e| S3Error(format!("hmac: {e}")))?;
    mac.update(data);
...
mod tests {
    ...
    #[test]
    fn amz_date_shape() {
        // 2026-08-20 00:00:00 UTC == 1787155200
        assert_eq!(amz_date_from_unix(1787155200), "20260820T000000Z");
